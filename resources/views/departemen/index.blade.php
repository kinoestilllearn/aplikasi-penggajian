...
<div class="space-y-6" x-data="{ openCreateModal: false, openEditModal: false, editId: null, editNama: '' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
        <div>
            <h1 class="text-xl font-extrabold text-[#1D1D1F] tracking-tight flex items-center gap-2">
                <i data-lucide="shield" class="w-6 h-6 text-[#0052CC]"></i>
                {{ $pageTitle }}
            </h1>
            <p class="text-xs text-[#86868B] mt-1">Kelola data divisi game (MLBB, PUBGM, Valorant, Content Creator) secara dinamis.</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-[#1D1D1F]">
                    ...
                    <tr class="hover:bg-[#FAFAFC] transition-colors">
                        ...
                        <td class="py-3.5 px-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-[#F2F2F7] text-slate-700">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-[#86868B] text-xs">Belum ada data divisi game.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div @click.away="openCreateModal = false" class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200/60 space-y-5">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 class="font-bold text-[#1D1D1F] text-base flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-5 h-5 text-[#0052CC]"></i> Tambah Divisi Game Baru
                </h3>
            ...
            <div>
                <label for="nama_create" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Nama Divisi Game</label>
    ...
    <div x-show="openEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-4">
        <div @click.away="openEditModal = false" class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200/60 space-y-5">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 class="font-bold text-[#1D1D1F] text-base flex items-center gap-2">
                    <i data-lucide="edit-3" class="w-5 h-5 text-[#0052CC]"></i> Ubah Divisi Game
                </h3>
            ...
            <form :action="'/departemen/' + editId" method="POST" class="space-y-4">
                ...
                @method('PUT')
                <div>
                    <label for="nama_edit" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Nama Divisi Game</label>
