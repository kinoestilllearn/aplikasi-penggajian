...
@php $pageTitle = 'EVOS Esport Team & Staff'; @endphp
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#151D2A] p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-colors duration-300">
        <div>
            <h1 class="text-xl font-extrabold text-[#0F172A] dark:text-[#F5F5F7] tracking-tight flex items-center gap-2">
                <i data-lucide="gamepad-2" class="w-6 h-6 text-[#0052CC] dark:text-[#00D2FF]"></i>
                {{ $pageTitle }}
            </h1>
            <p class="text-xs text-slate-600 dark:text-[#94A3B8] font-medium mt-1">Kelola data player, coach, analyst, manager per Divisi Game, serta komponen finansial (Gaji Pokok & Tunjangan Tetap).</p>
        </div>

        <div class="flex items-center gap-3">
            <span class="px-3 py-1.5 bg-blue-50 dark:bg-[#1C2536] text-[#0052CC] dark:text-[#00D2FF] rounded-full text-xs font-bold flex items-center gap-1.5 border border-blue-100 dark:border-blue-900/40">
                <i data-lucide="shield-check" class="w-3.5 h-3.5 text-[#0052CC] dark:text-[#00D2FF]"></i> EVOS HQ Live Database
            </span>
