...
<div class="space-y-6" x-data="{ openCreateModal: false, openEditModal: false, editId: null, editNama: '', editDepartemenId: '' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#151D2A] p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-colors duration-300">
        <div>
            <h1 class="text-xl font-extrabold text-[#0F172A] dark:text-[#F5F5F7] tracking-tight flex items-center gap-2">
                <i data-lucide="swords" class="w-6 h-6 text-[#0052CC] dark:text-[#00D2FF]"></i>
                {{ $pageTitle }}
            </h1>
            <p class="text-xs text-slate-600 dark:text-[#94A3B8] font-medium mt-1">Kelola posisi dan role pekerjaan roster (Player, Coach, Analyst, Manager) per Divisi Game.</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800 text-sm text-[#0F172A] dark:text-[#F5F5F7]">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-[#1E293B] transition-colors">
                        ...
                        <td class="py-3.5 px-4">
                            <span class="px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-[#1C2536] text-[#0052CC] dark:text-[#00D2FF] text-xs font-bold border border-blue-100 dark:border-blue-900/40">
                                {{ $pos->departemen?->nama ?? '-' }}
                            </span>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-slate-500 dark:text-[#94A3B8] text-xs font-medium">Belum ada data role / position.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div @click.away="openCreateModal = false" class="bg-white dark:bg-[#151D2A] rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                <h3 class="font-bold text-[#0F172A] dark:text-[#F5F5F7] text-base flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-5 h-5 text-[#0052CC] dark:text-[#00D2FF]"></i> Tambah Role Baru
                </h3>
            ...
            <div>
                <label for="nama_pos_create" class="block text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider mb-1">Nama Role / Position</label>
            ...
            <div>
                <label for="dep_pos_create" class="block text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider mb-1">Pilih Divisi Game</label>
    ...
    <div x-show="openEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
        <div @click.away="openEditModal = false" class="bg-white dark:bg-[#151D2A] rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5">
            <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                <h3 class="font-bold text-[#0F172A] dark:text-[#F5F5F7] text-base flex items-center gap-2">
                    <i data-lucide="edit-3" class="w-5 h-5 text-[#0052CC] dark:text-[#00D2FF]"></i> Ubah Role / Position
                </h3>
            ...
            <form :action="'/posisi/' + editId" method="POST" class="space-y-4">
                ...
                @method('PUT')
                <div>
                    <label for="nama_pos_edit" class="block text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider mb-1">Nama Role / Position</label>
                    <input type="text" name="nama" id="nama_pos_edit" required x-model="editNama"
                        class="w-full px-3.5 py-2.5 bg-white dark:bg-[#1C2536] border border-slate-300 dark:border-slate-700 rounded-xl text-sm font-semibold text-[#0F172A] dark:text-[#F5F5F7] focus:border-[#0052CC] outline-none">
                </div>

                <div>
                    <label for="dep_pos_edit" class="block text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider mb-1">Pilih Divisi Game</label>
