    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('status', function (Penggajian $penggajian) {
                ...
                if ($status === 'disetujui') {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200/60"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Disetujui</span>';
                } elseif ($status === 'dibatalkan') {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200/60"><span class="w-1.5 h-1.5 rounded-full bg-rose-500"></span>Dibatalkan</span>';
                } else {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200/60"><span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>Draf</span>';
                }
            })
            ->addColumn('pegawai', function (Penggajian $penggajian) {
                return '<span class="font-bold text-[#1D1D1F]">' . e($penggajian->pegawai?->nama ?? '-') . '</span>';
            })
            ->addColumn('posisi', function (Penggajian $penggajian) {
                return '<span class="font-semibold text-slate-700">' . e($penggajian->pegawai?->posisi?->nama ?? '-') . '</span>';
            })
            ->addColumn('departemen', function (Penggajian $penggajian) {
                return '<span class="px-2.5 py-1 rounded-lg bg-[#F2F2F7] text-[#0052CC] text-xs font-bold">' . e($penggajian->pegawai?->departemen?->nama ?? '-') . '</span>';
            })
            ->addColumn('dibuat_oleh', function (Penggajian $penggajian) {
                return e($penggajian->dibuatOleh?->name ?? '-');
            })
            ->addColumn('disetujui_oleh', function (Penggajian $penggajian) {
                return e($penggajian->disetujuiOleh?->name ?? '-');
            })
            ->addColumn('dibatalkan_oleh', function (Penggajian $penggajian) {
                return e($penggajian->dibatalkanOleh?->name ?? '-');
            })
            ->addColumn('gaji_pokok', function (Penggajian $penggajian) {
                $gajiPokok = $penggajian->pegawai?->gaji_pokok ?? 0;
                return '<span class="font-bold text-[#1D1D1F]">Rp ' . number_format($gajiPokok, 0, ',', '.') . '</span>';
            })
            ...
            ->rawColumns(['status', 'pegawai', 'posisi', 'departemen', 'gaji_pokok', 'action']);
    }
