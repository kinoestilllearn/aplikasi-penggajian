        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('nama', function (Pegawai $pegawai) {
                ...
                return '<div class="flex items-center gap-3">
                    <img src="' . $avatarUrl . '" class="w-9 h-9 rounded-xl shadow-sm border border-blue-200 shrink-0" alt="Player Avatar">
                    <div>
                        <div class="font-extrabold text-[#0F172A] dark:text-[#F5F5F7] leading-snug">' . e($pegawai->nama) . '</div>
                        <div class="text-[11px] text-[#0052CC] dark:text-[#00D2FF] font-mono font-bold">ID: ' . e($pegawai->no_pegawai) . '</div>
                    </div>
                </div>';
            })
            ->editColumn('jenis_kelamin', function (Pegawai $pegawai) {
                return $pegawai->jenis_kelamin == 'L'
                    ? '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-100/90 text-[#0052CC] border border-blue-200 dark:bg-blue-900/40 dark:text-[#00D2FF] dark:border-blue-800">Laki-Laki</span>'
                    : '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-pink-100/90 text-pink-800 border border-pink-200 dark:bg-pink-900/40 dark:text-pink-300 dark:border-pink-800">Perempuan</span>';
            })
            ->editColumn('status_pegawai', function (Pegawai $pegawai) {
                $status = strtolower($pegawai->status_pegawai);
                if ($status === 'tetap') {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100/90 text-[#0052CC] border border-blue-200 dark:bg-blue-900/40 dark:text-[#00D2FF] dark:border-blue-800"><span class="w-1.5 h-1.5 rounded-full bg-[#0052CC] dark:bg-[#00D2FF] animate-pulse"></span>Tetap</span>';
                } elseif ($status === 'kontrak') {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-cyan-100/90 text-cyan-900 border border-cyan-300 dark:bg-cyan-900/40 dark:text-cyan-300 dark:border-cyan-800"><span class="w-1.5 h-1.5 rounded-full bg-cyan-600"></span>Kontrak</span>';
                } else {
                    return '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100/90 text-amber-900 border border-amber-300 dark:bg-amber-900/40 dark:text-amber-300 dark:border-amber-800"><span class="w-1.5 h-1.5 rounded-full bg-amber-600"></span>Harian Lepas</span>';
                }
            })
            ->editColumn('gaji_pokok', function (Pegawai $pegawai) {
                return '<span class="font-extrabold text-[#0F172A] dark:text-[#F5F5F7]">Rp ' . number_format($pegawai->gaji_pokok ?? 0, 0, ',', '.') . '</span>';
            })
            ->editColumn('tunjangan_tetap', function (Pegawai $pegawai) {
                return '<span class="font-bold text-slate-800 dark:text-slate-300">Rp ' . number_format($pegawai->tunjangan_tetap ?? 0, 0, ',', '.') . '</span>';
            })
            ->addColumn('departemen', function (Pegawai $pegawai) {
                return '<span class="px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-[#1C2536] text-[#0052CC] dark:text-[#00D2FF] text-xs font-bold border border-blue-100 dark:border-blue-900/40">' . e($pegawai->departemen?->nama ?? '-') . '</span>';
            })
            ->addColumn('posisi', function (Pegawai $pegawai) {
                return '<span class="font-bold text-[#0F172A] dark:text-slate-200">' . e($pegawai->posisi?->nama ?? '-') . '</span>';
            })
